...
{
    /// <summary>
    /// Tooltip flutuante que mostra informações de um item.
    /// Pode ser ancorado em um RectTransform (slot) ou seguir o cursor.
    ///
    /// Seções condicionais:
    ///   - Stats (apenas equipamento com bônus)
    ///   - Requisitos (nível, atributos, raça)
    ///   - Skill (apenas PowerGem)
    ///   - Consumable (apenas Consumable)
    /// </summary>
    public class ItemTooltipUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// Busca atributos totais do jogador local. Usa GetRaceEnum() do
        /// NetworkPlayer (cache) em vez de Enum.Parse.
        /// </summary>
        private static bool TryGetLocalPlayerStats(
            out int level, out int str, out int agi, out int vit,
            ...
            else
            {
                ...
                if (cam != null)
                {
                    Vector3 worldPos = cam.ScreenToWorldPoint(new Vector3(
                        mousePos.x, mousePos.y, canvas.planeDistance));
                    _root.position = worldPos;
                }
            }
